...
<h1>Revivir pallet</h1>
<table>
    <tr>
        <td><a href='index.php'><button type='button'>Inicio</button></a></td>
        <td><input type="checkbox" id="myCheckbox"></td>
        <td><form action='' method='post'><input type='submit' name='revivir' value='Revivir!' id="myButton" disabled></form></td>
    </tr>
</table>
...
<?php
    ...
    if (empty($registros)){
        ...
        echo "<table><tr><form action='revivir.php' method='GET'>";
        ...
        if (empty($registros_pallets)){
            ...
            if(isset($_POST['revivir'])){
                ...
                $query_update_pallet="UPDATE produccion_general SET estado ='D', pedido='0' WHERE codbar_s= ?";
                ...
                $query_update_pallets="UPDATE pallets SET SESTADO='D' WHERE INCPALLET= ?";
                $resul_update_pallets=$base->prepare($query_update_pallets);
                $resul_update_pallets->execute(array($pallet));
                $resul_update_pallets->closeCursor();
    
                $query_update_codbars="UPDATE produccion_general SET estado ='PA', incpallet ='$pallet', pedido='0' WHERE codbar_s in $codbars";
                ...
                if($estado="E"){
                    ...
                    $query_update_expedicion="UPDATE expedicion_colector_detalle SET bandera='NO' WHERE codigo= ?";
                    $resul_update_expedicion=$base->prepare($query_update_expedicion);
                    $resul_update_expedicion->execute(array($pallet));
                    $resul_update_expedicion->closeCursor();
    
                }
    
                header("Location: revivir.php?pallet=$pallet");
...
<script>
    ...
    if (this.checked) {
        ...
        button.innerText = "Botón activado";
    } else {
        button.disabled = true;
        button.innerText = "Botón desactivado";
    }
    });
</script>
</body>
